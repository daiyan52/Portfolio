      ...
      <div className="flex flex-wrap  text-xl font-serif justify-center mt-5">
        ...
        {githubRepos.length > 0 && (
          <div className="w-full mt-5">
            <h3 className="font-semibold text-xl text-center text-gray-800 mb-4">Repositories:</h3>
            <div className="flex flex-wrap justify-center">
              {githubRepos.map((repo, index) => (
                <div key={repo.id} className="max-w-xs w-full p-4 sm:max-w-sm md:max-w-md lg:max-w-lg">
                  <div className={`bg-${index % 2 === 0 ? 'blue-100' : 'green-100'} border rounded-lg shadow-md overflow-hidden h-full`}>
                    <div className="p-4 sm:p-6 flex flex-col justify-between h-full">
                      <h4 className="font-semibold text-gray-800 text-lg sm:text-xl mb-2">{repo.name}</h4>
                      <p className="text-gray-700 mb-4 text-sm sm:text-base">{repo.description || "No description available."}</p>
                    
                      <p className="text-gray-500 text-sm sm:text-base">Language: {repo.language || "N/A"}</p>
                      <div className="mt-2">
                        <a
                          href={repo.html_url}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="text-blue-500 text-lg sm:text-xl hover:underline mr-4"
                        >
                          <i className="fa fa-github mr-1"></i> View Repo
                        </a>
                      </div>
                    </div>
                  </div>
